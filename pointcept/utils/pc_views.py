...
class PCViews:
    ...
    def get_img_and_index(self, points):
        """
        返回深度图 + 索引图
        Args:
            points: [B, N, 3]
        Returns:
            depth:  [B*6, 512, 512]
            index:  [B*6, 512, 512]   LongTensor, 值是原始点云中的点id, -1表示无点
        """
        b, n, _ = points.shape
        v = self.num_views
        ...
        # Step 1: 复制点云
        points_expanded = torch.repeat_interleave(points, v, dim=0)  # [B*6, N, 3]
        
        # Step 2: 变换到相机坐标系
        points_cam = self.point_transform(
            points=points_expanded,
            rot_mat=self.rot_mat.repeat(b, 1, 1),
            translation=self.translation.repeat(b, 1, 1)
        )  # [B*6, N, 3]
        
        # Step 3: 透视投影 - 修正坐标计算以匹配原始points2depth函数
        eps = 1e-8
        z = points_cam[:, :, 2]
        
        # 关键修正：与原始points2depth函数保持一致
        # 原始实现中x坐标会乘以 (image_width/image_height)
        coord_x = (points_cam[:, :, 0] / (z + eps)) * (W / H)  # [batch, num_points]
        coord_y = (points_cam[:, :, 1] / (z + eps))  # [batch, num_points]
        
        # 像素坐标 - 注意：x对应高度方向，y对应宽度方向
        px = ((coord_x + 1) * H) / 2  # 注意这里使用H，与原始实现一致
        py = ((coord_y + 1) * W) / 2  # 注意这里使用W，与原始实现一致
        
        # 为所有点创建批次索引和点ID
        batch_idx = torch.arange(b*v, device=points.device).view(-1, 1).expand(-1, n)
        point_ids = torch.arange(n, device=points.device).view(1, -1).expand(b*v, -1)
        
        # 有效点掩码: 只考虑相机前方且在图像范围内的点
        valid_mask = (z > eps) & (px >= 0) & (px < H) & (py >= 0) & (py < W)
        
        # 初始化深度图和索引图
        large_value = 1e9
        depth_map = torch.full((b*v, H, W), large_value, device=points.device)
        index_map = torch.full((b*v, H, W), -1, device=points.device, dtype=torch.long)
        
        # 提取有效点
        valid_px = px[valid_mask].round().long().clamp(0, H-1)
        valid_py = py[valid_mask].round().long().clamp(0, W-1)
        valid_z = z[valid_mask]
        valid_batch = batch_idx[valid_mask]
        valid_point_ids = point_ids[valid_mask]
        
        # 按深度升序排序，这样近的点会覆盖远的点 - 修正排序方向
        sort_indices = torch.argsort(valid_z)
        valid_px = valid_px[sort_indices]
        valid_py = valid_py[sort_indices]
        valid_z = valid_z[sort_indices]
        valid_batch = valid_batch[sort_indices]
        valid_point_ids = valid_point_ids[sort_indices]
        
        # 更新深度图和索引图
        depth_map[valid_batch, valid_px, valid_py] = valid_z  # 注意这里索引顺序
        index_map[valid_batch, valid_px, valid_py] = valid_point_ids
        
        # 将没有点的像素深度设为0
        no_point_mask = (depth_map > large_value / 2)
        depth_map[no_point_mask] = 0

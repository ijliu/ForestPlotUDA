...
@MODELS.register_module("Sonata-Pseudo")
class UDASonata(PointModel):
    def __init__(
        self,
        backbone,
        head_in_channels,
    ...
    def forward(self, epoch, train_data_dict, aux_data_dict, pseudo_input_dict, return_point=False):
        ######### 测试部分 ###########
        if return_point:
            with torch.no_grad():
                point = Point(epoch)
                point = self.student.backbone(point)
        ...
        loss_sup = self.criteria(seg_logits, train_data_dict["segment"].long()) * W1
        result_dict["loss_sup"] = loss_sup
        result_dict["loss"].append(loss_sup)
        ###########################################
        # ===== 2. 真实数据自监督（Sonata） =====
        #############################################
        data_dict = aux_data_dict
        # prepare global_point, mask_global_point, local_point
        with torch.no_grad():
            # global_point & masking
            global_point = Point(
        ...
        if epoch > 100:
            ...
            if isinstance(point, Point):
                ...
                feat = point
            out_real = self.seg_head(feat)
            loss_pseudo = (F.cross_entropy(out_real, pseudo, reduction='none')).mean() * W3

            alpha = min(1.0, 0.1 + 0.09 * (epoch - 100))
            result_dict["loss_pseudo"] = loss_pseudo
            result_dict["loss"].append(loss_pseudo * alpha)

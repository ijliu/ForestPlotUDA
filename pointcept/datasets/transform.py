...
class CylinderCrop(object):
    def __init__(self, radius=4, point_max=None, mode="random"):
        self.radius = radius
        self.point_max = point_max
        assert mode in ["random", "center", "all"], "mode must be 'random', 'center', 'all' or 'grid'"
        self.mode = mode

    def __call__(self, data_dict):
        assert "coord" in data_dict.keys(), "data_dict must contain 'coord' key"
        coords = data_dict["coord"] 
        n_points = coords.shape[0]
        if n_points == 0:
            return [data_dict] if self.mode == "grid" else data_dict  

        xy_coords = coords[:, :2]  # 提取x-y坐标

        if self.mode == "random":
            center_idx = np.random.randint(n_points)
            center_xy = xy_coords[center_idx]  # (x0, y0)
            kd_tree = KDTree(xy_coords)
            idx_crop = kd_tree.query_ball_point(center_xy, r=self.radius)
            idx_crop = np.array(idx_crop, dtype=int)
            ...
            if 'instance' in data_dict:
                ...
                non_zero = inst != 0
                unique_vals, inverse = np.unique(inst[non_zero], return_inverse=True)
                new_inst = np.zeros_like(inst)
                new_inst[non_zero] = inverse + 1
                data_dict['instance'] = new_inst
            return data_dict

...
@HOOKS.register_module()
class UDACheckpointLoader(HookBase):
    def __init__(self, keywords="", replacement=None, strict=False):
        self.keywords = keywords
        self.replacement = replacement if replacement is not None else keywords
        self.strict = strict
    ...
    def before_train(self):
        self.trainer.logger.info("=> Loading checkpoint & weight ...")
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            self.trainer.logger.info(f"Loading student's weight at: {self.trainer.cfg.weight}")
            checkpoint = torch.load(
                self.trainer.cfg.weight,
                map_location=lambda storage, loc: storage.cuda(),
                weights_only=False
            ...
            for key, value in checkpoint["state_dict"].items():
                ...
                # if 'norm.0' in key:
                #     for i in range(5):
                #         new_key = key.replace('norm.0', f"norm.0.norm.{i}")
                #         weight[new_key] = value
                # elif 'stem.norm' in key:
                #     for i in range(5):
                #         new_key = key.replace('stem.norm', f"stem.norm.norm.{i}")
                #         weight[new_key] = value
                # elif 'proj.1' in key:
                #     for i in range(5):
                #         new_key = key.replace('proj.1', f"proj.1.norm.{i}")
                #         weight[new_key] = value
                # elif 'proj_skip.1' in key:
                #     for i in range(5):
            ...
            ## Teacher
            self.trainer.logger.info(f"Loading teacher's weight at: {self.trainer.cfg.weight}")
            load_state_info = self.trainer.teacher.load_state_dict(
                weight, strict=self.strict
            )
            self.trainer.logger.info(f"Missing keys: {load_state_info[0]}")
        else:
        ...
        # for name,p in self.trainer.teacher.named_parameters():
        #     print(name)
        # exit()
